...
оценка движения камеры, имеющей шесть степеней свободы, на основании данных из видеопотока [1-6].
...
Другие направления использования &mdash; это виртуальная или дополненая реальность,
...
Для методов, использующих локальные особенности изображения,
эта задача оптимизации обычно известна как
"bundle adjustment" [7] и по ней наработано множество решений,
эффективно переводящих её в нелинейную задачу наименьших квадратов [8-11].
Три аспекта являются ключевыми для получения высокой точности при использовании разрежённого соответствия особенностей и совместной оптимизации: 
...
3. Достоверное сопоставление новых особенностей ранее известным местам (то есть, замыкание циклов).
...
задачи. Однако, совместная оптимизация плотной структуры
...
извлечение описателей, что выливается в высокие постоянные затраты на обработку каждого кадра.

В данной работе, мы представляем алгоритм визуальной одометрии (VO) который объединяет
преимущества прямых методов и методов, использующих особенности.
...
работающий путём минимизации фотометрической ошибки при сопоставлении особенностей,
лежащих на углах и краях интенсивности.
Точки в трёхмерной карте, соответствующие
особенностям, определяются через устойчивую к ошибкам рекурсивную процедуру Баесовской
оценки глубины ([раздел VI](#v-Построение-карты "Построение карты")). Как только соответствие особенностей
...
- во-первых, SVO извлекает особенности только из отдельных
ключевых кадров, и делает это в параллельном потоке, отвязанном от
...
заключается в обобщении метода на объективы с широким углом обзора ([раздел VII](#vii-Использование-камер-с-широким-углом-обзора "Использование камер с широким углом обзора")),
...
*1) Использующие особенности.* Типичный метод решения этой
задачи состоит следующих шагов:
а) выделить разрежённое множество заметных особенностей изображения
(например углов и пятен) на каждом кадре; 
б) найти соответствие между ними на последовательных кадрах,
используя инвариантные описатели особенностей; 
в) пользуясь эпиполярной геометрией, восстановить движение камеры и структуру сцены,
отсекая ложные соответствия;
г) и наконец, оптимизировать положение камеры и структуру через минимизацию
ошибки репроекции. 
Этой процедуре (с некоторыми изменениями) следует большинство
...
достаточно больших различиях освещения и места съёмки. Описатели особенностей
также могут быть использованы для поиска соответствий со
старыми отметками при замыкании циклов, что увеличивает как
точность определения траектории после совместной оптимизации (bundle adjustment) [7, 21], так и устойчивость к ошибкам всей системы из-за возможностей
...
$$u \in R_{k}^{C} $$ положительна, положение точки в пространстве определяется с помощью
модели обратной проекции $$ \rho = \pi_{\rho}^{−1} (u) $$. Если мы обозначили
$$ R_{k}^{C} \subseteq \Omega $$, для тех пикселей известна глубина на шаге времени
...
проблемы наименьших квадратов, например Левенберга — Марквардта
(Levenberg-Marquardt). Больше подробностей по оптимизации,
